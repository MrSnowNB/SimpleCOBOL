...
Reads: app/cbl/<PROG>.cbl + validation/rekt/<PROG>.cbl.report/cfg/cfg-<PROG>.cbl.json
Writes: poc/facts/<PROG>.json
...
SRC_DIR  = Path("app/cbl")
CPY_DIR  = Path("app/cpy")
REKT_DIR = Path("validation/rekt")
OUT_DIR  = Path("poc/facts")
...
def cobc_expand(prog: str) -> list:
    ...
    # strip preprocessor line markers (#line ...)
...
def rekt_sentences(rekt: dict) -> list:
    return [
        ...
        if n.get("nodeType") == "CODE_VERTEX"
           and n.get("originalText","").strip()
    ]
...
def rekt_calls(sentences: list) -> list:
    ...
    return list(dict.fromkeys(calls))  # deduplicated, ordered
...
# -- 4. Data item extraction (top-level 01s only for POC) --------------------
...
def assemble(prog: str):
    ...
    calls     = rekt_calls(sentences)
    cics      = rekt_cics(sentences)
...
if __name__ == "__main__":
    progs = sys.argv[1:] if len(sys.argv) > 1 else [
        p.stem for p in SRC_DIR.glob("*.cbl")
    ]
